...
// Make changes to `perlin.wgsl` instead and rebuild
...
export default () => `
  ...
  fn perlin2D(posX: f32, posY: f32) -> f32 {
    let batchX = i32(floor(posX / f32(uniforms.k)));
    let batchY = i32(floor(posY / f32(uniforms.k)));
    ...
    let x = posX / f32(uniforms.k) - f32(batchX);
    let y = posY / f32(uniforms.k) - f32(batchY);
  ...
  fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let idx = global_id.x + global_id.y * uniforms.n;
